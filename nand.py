...
# Buttons
btn_attack = telebot.types.KeyboardButton("Server Hack 🚨")
btn_start = telebot.types.KeyboardButton("FUCK LOBBY 💦")
btn_stop = telebot.types.KeyboardButton("STOP BABY 😺")
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By @CREATIVEYDV✅"
...
@bot.message_handler(func=lambda message: message.text == 'Server Hack 🚨')
...
@bot.message_handler(func=lambda message: message.text == 'FUCK LOBBY 💦')
def handle_start_attack(message):
    try:
        ...
        if attack_details:
            ...
            bot.send_message(chat_id, "Please set the target IP and Port first by using 'Server Hack 🚨'", parse_mode='Markdown')
    except Exception as e:
        logging.error(f"Error in start_attack: {e}")

# Function to stop the attack
@bot.message_handler(func=lambda message: message.text == 'STOP BABY 😺')
